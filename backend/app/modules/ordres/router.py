...
# URL du système MES (à adapter selon votre déploiement)
...
# ✅ Créer un OF côté ERP (sans l'envoyer encore)
...
# ✅ Modifier un OF (seulement si Brouillon)
...
# ✅ Supprimer un OF (seulement si Brouillon)
@router.delete("/{of_id}")
...
# ─────────────────────────────────────────────────────────────────────────────
# 🚀 ENVOI VERS MES (communication API REST)
# ─────────────────────────────────────────────────────────────────────────────

@router.post("/{of_id}/envoyer-mes")
async def envoyer_vers_mes(of_id: int, db: Session = Depends(get_db)):
    """
    Envoie un OF depuis l'ERP vers le MES via une requête HTTP POST.
    Met à jour le statut de l'OF à 'Envoyé' en cas de succès.
    """
    of = db.query(OFErp).filter(OFErp.id == of_id).first()
    if not of:
        raise HTTPException(status_code=404, detail="OF non trouvé")
    ...
    if of.statut_erp == "Envoyé":
        raise HTTPException(
            status_code=400,
            detail="Cet OF a déjà été envoyé au MES"
        )

    # Prépare le payload pour le MES
    payload = {
        "numero": of.numero,
        "machine": of.machine,
        "produit": of.produit,
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            # OF déjà présent dans le MES
            of.statut_erp = "Envoyé"
            db.commit()
            return {
                "success": True,
                "message": f"OF {of.numero} déjà présent dans le MES (synchronisé)",
            }
        else:
            raise HTTPException(
                status_code=502,
                detail=f"Erreur MES ({response.status_code}): {response.text}"
            )

    except httpx.ConnectError:
        raise HTTPException(
            status_code=503,
            detail="Impossible de joindre le système MES. Vérifiez la connexion."
        )
    except httpx.TimeoutException:
        raise HTTPException(
            status_code=504,
            detail="Le système MES n'a pas répondu dans les délais."
        )


# ─────────────────────────────────────────────────────────────────────────────
# 🚀 ENVOI EN MASSE vers MES
# ─────────────────────────────────────────────────────────────────────────────

@router.post("/envoyer-tous-mes")
async def envoyer_tous_vers_mes(db: Session = Depends(get_db)):
    """Envoie tous les OFs en statut Brouillon vers le MES."""

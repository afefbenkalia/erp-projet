// Approvisionnement.jsx
// Module ERP – Approvisionnement : Fournisseurs + Commandes Achat + Réceptions
...
// ─── Design System (cohérent avec le reste de l'ERP) ──────────────────────
const C = {
  bg: "#f4f6f9",
  surface: "#ffffff",
  border: "#e3e8ef",
...
const s = {
  ...
  toast: {
    ...
    padding: "0.9rem 1.4rem", borderRadius: 10,
    fontWeight: 500, fontSize: "0.95rem", zIndex: 9999,
  ...
  modal: {
    ...
    padding: "2rem", maxWidth: 440, width: "90%",
...
const STATUT_STYLE = {
  "Brouillon":        { bg: "#dbeafe", color: "#1e40af", border: "#bfdbfe", icon: "✏️" },
  "Envoyée":          { bg: "#fef9c3", color: "#854d0e", border: "#fde68a", icon: "📤" },
  "Reçue partielle":  { bg: "#fed7aa", color: "#9a3412", border: "#fdba74", icon: "⚠️" },
  "Reçue totale":     { bg: "#d1fae5", color: "#065f46", border: "#6ee7b7", icon: "✅" },
  "Annulée":          { bg: "#fee2e2", color: "#991b1b", border: "#fca5a5", icon: "❌" },
};
...
const Approvisionnement = () => {
  ...
  const [showReception, setShowReception] = useState(null); // commande sélectionnée
  ...
  const handleSubmitCmd = async () => {
    ...
    try {
      const payload = {
        ...
        prix_unitaire: formCmd.prix_unitaire ? parseFloat(formCmd.prix_unitaire) : null,
        date_commande: formCmd.date_commande || null,
        date_livraison_prevue: formCmd.date_livraison_prevue || null,
      };
      if (editCmd) {
        await axios.put(`${BASE}/commandes/${editCmd.id}`, payload);
        showToast(`Commande ${formCmd.numero} mise à jour`);
      } else {
        await axios.post(`${BASE}/commandes`, payload);
        showToast(`Commande ${formCmd.numero} créée`);
      }
  ...
  const handleEnvoyer = async (c) => {
    try {
      ...
      showToast(`Commande ${c.numero} envoyée au fournisseur`);
      loadAll();
  ...
  const handleAnnuler = async () => {
    try {
      ...
      showToast(`Commande ${confirmAnnuler.numero} annulée`);
  ...
  const handleDeleteCmd = async () => {
    try {
      ...
      showToast(`Commande ${confirmDelete.numero} supprimée`);
  ...
  const openEditCmd = (c) => {
    ...
    setFormCmd({
      ...
      unite: c.unite,
      prix_unitaire: c.prix_unitaire ? String(c.prix_unitaire) : "",
  ...
  const handleReception = async () => {
    if (!formRec.quantite_recue || !formRec.date_reception) {
      return showToast("Remplissez quantité et date de réception", "error");
    }
    setLoading(true);
    try {
      ...
      showToast(`Réception enregistrée — stock mis à jour automatiquement ✅`);
      setShowReception(null);
      setFormRec({ quantite_recue: "", date_reception: "", commentaire: "" });
      loadAll();
    } catch (err) {
      showToast(err.response?.data?.detail || "Erreur réception", "error");
    } finally { setLoading(false); }
  };
  ...
  const handleSubmitFour = async () => {
    if (!formFour.code || !formFour.nom) {
      return showToast("Code et nom sont obligatoires", "error");
    }
    setLoading(true);
    try {
      if (editFour) {
        await axios.put(`${BASE}/fournisseurs/${editFour.id}`, formFour);
        showToast(`Fournisseur ${formFour.nom} mis à jour`);
      } else {
        await axios.post(`${BASE}/fournisseurs`, formFour);
        showToast(`Fournisseur ${formFour.nom} créé`);
      }
      setShowFormFour(false); setEditFour(null);
      setFormFour({ code: "", nom: "", contact: "", telephone: "", email: "", adresse: "", article_code: "", article_nom: "", delai_livraison_jours: 7, actif: "Actif" });
      loadAll();
    } catch (err) {
      showToast(err.response?.data?.detail || "Erreur", "error");
    } finally { setLoading(false); }
  ...
  const handleDeleteFour = async (f) => {
    ...
    try {
      ...
      showToast(`Fournisseur ${f.nom} supprimé`);
      loadAll();
    } catch (err) { showToast(err.response?.data?.detail || "Erreur", "error"); }
  };
  ...
  const kpis = [
    { label: "Total commandes", val: dashboard?.total_commandes ?? 0, color: C.teal, lt: C.tealLt, icon: "🛒" },
  ...
  return (
    <div style={s.app}>
      ...
      {toast && (
        <div style={{
          ...s.toast,
          background: toast.type === "error" ? C.redLt : C.greenLt,
      ...
      <div style={s.topbar}>
        ...
        <div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
          <span style={{ background: C.tealLt, color: C.teal, border: `1px solid #99f6e4`, borderRadius: 20, padding: "3px 10px", fontSize: "0.72rem", fontWeight: 600 }}>
            {dashboard?.fournisseurs_actifs ?? 0} fournisseurs actifs
          </span>
        </div>
      </div>
      ...
      <div style={s.main}>
          ...
          <>
              ...
              <div style={s.section}>
                <div style={s.sectionHeader}>
                  <div>
                    ...
                    <p style={s.sectionSub}>Numéro généré automatiquement · Stock mis à jour à la réception</p>
                  </div>
                ...
                <div style={s.grid3}>
                  <div style={s.fg}>
                    ...
                    <input style={{ ...s.input, fontFamily: "monospace", fontWeight: 600, background: "#f8fafc" }}
                  ...
                  <div style={s.fg}>
                    <label style={s.label}>Prix unitaire (DT/kg)</label>
                    <input style={s.input} type="number" min="0" step="0.001" placeholder="0.000"
                      value={formCmd.prix_unitaire}
                      onChange={(e) => setFormCmd({ ...formCmd, prix_unitaire: e.target.value })} />
                  </div>
                  <div style={s.fg}>
                    <label style={s.label}>Date commande</label>
                    <input style={s.input} type="date" value={formCmd.date_commande}
                      onChange={(e) => setFormCmd({ ...formCmd, date_commande: e.target.value })} />
                  </div>
                  <div style={s.fg}>
                    <label style={s.label}>Date livraison prévue</label>
                    <input style={s.input} type="date" value={formCmd.date_livraison_prevue}
                      onChange={(e) => setFormCmd({ ...formCmd, date_livraison_prevue: e.target.value })} />
                  </div>
                ...
                {formCmd.quantite_commandee && formCmd.prix_unitaire && (
                  <div style={{ marginTop: "1rem", padding: "0.75rem 1rem", background: C.tealLt, borderRadius: 8, border: `1px solid #99f6e4`, fontSize: "0.9rem", color: C.teal, fontWeight: 600 }}>
                    💰 Montant total estimé : {fmt(parseFloat(formCmd.quantite_commandee) * parseFloat(formCmd.prix_unitaire))} DT
                  </div>
                )}
            ...
            <div style={s.section}>
              ...
              <div style={{ display: "flex", gap: "0.75rem", marginBottom: "1.25rem", flexWrap: "wrap" }}>
                <input style={{ ...s.input, maxWidth: 220 }} placeholder="🔍 Rechercher N° ou article..."
                  value={searchCmd} onChange={(e) => setSearchCmd(e.target.value)} />
                <select style={{ ...s.select, maxWidth: 180 }} value={filterStatut} onChange={(e) => setFilterStatut(e.target.value)}>
                  <option value="">Tous les statuts</option>
                  {Object.keys(STATUT_STYLE).map((s_) => (
                    <option key={s_} value={s_}>{STATUT_STYLE[s_].icon} {s_}</option>
                  ))}
                </select>
                <select style={{ ...s.select, maxWidth: 200 }} value={filterFour} onChange={(e) => setFilterFour(e.target.value)}>
              ...
              {filtered.length === 0 ? (
                ...
                <div style={{ overflowX: "auto" }}>
                  <table style={s.table}>
                    <thead style={s.thead}>
                      <tr>
                        {["N° Commande", "Fournisseur", "Article", "Qté commandée", "Reçue", "Avancement", "Livraison prévue", "Statut", "Actions"].map((h) => (
                    ...
                    <tbody>
                      {filtered.map((c, i) => {
                        const st = STATUT_STYLE[c.statut] || { bg: "#f1f5f9", color: "#475569", border: "#e2e8f0", icon: "?" };
                        const pct = c.quantite_commandee > 0
                          ? Math.min(100, Math.round((c.quantite_recue / c.quantite_commandee) * 100))
                          : 0;
                        const isRetard = c.statut === "Envoyée" && c.date_livraison_prevue &&
                          new Date(c.date_livraison_prevue) < new Date();
                        return (
                          <tr key={c.id} style={{ background: i % 2 === 0 ? "#fff" : C.bg }}>
                            <td style={s.td}>
                              <code style={{ color: C.teal, fontSize: "0.8rem", fontFamily: "monospace", background: C.tealLt, padding: "1px 6px", borderRadius: 4 }}>
                                {c.numero}
                              </code>
                            </td>
                            ...
                            <td style={{ ...s.td, minWidth: 120 }}>
                              ...
                              <div style={s.progressBar(pct, pct === 100 ? C.green : C.teal)} />
                            </td>
                            <td style={{ ...s.td, color: isRetard ? C.red : C.sub, fontWeight: isRetard ? 600 : 400 }}>
                              {c.date_livraison_prevue || "—"}
                              {isRetard && <span style={{ marginLeft: 4, fontSize: "0.75rem" }}>⚠ Retard</span>}
                            </td>
                            <td style={s.td}>
                              ...
                              <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                                {c.statut === "Brouillon" && (
                                  <>
                                    ...
                                    <button style={s.btnAction(C.tealLt, C.teal, "#99f6e4")} onClick={() => handleEnvoyer(c)}>📤 Envoyer</button>
                                ...
                                {c.statut === "Reçue totale" && (
                                  <span style={{ color: C.green, fontSize: "0.8rem" }}>Reçue ✓</span>
                                )}
                              </div>
                            </td>
                          </tr>
              ...
              <div style={s.section}>
                ...
                <div style={s.grid3}>
                  ...
                  <div style={s.fg}>
                    ...
                    <input style={s.input} placeholder="+216 XX XXX XXX" value={formFour.telephone}
                      onChange={(e) => setFormFour({ ...formFour, telephone: e.target.value })} />
                  </div>
                  <div style={s.fg}>
                  ...
                  <table style={s.table}>
                    ...
                    <tbody>
                      {fournisseurs.map((f, i) => (
                        <tr key={f.id} style={{ background: i % 2 === 0 ? "#fff" : C.bg }}>
                          <td style={s.td}>
                            <code style={{ color: C.teal, fontSize: "0.8rem", background: C.tealLt, padding: "1px 6px", borderRadius: 4 }}>
                              {f.code}
                            </code>
                          </td>
                          ...
                          <td style={s.td}>
                            <div style={{ display: "flex", gap: 4 }}>
                              <button style={s.btnGhost} onClick={() => {
                                setEditFour(f);
                                setFormFour({ code: f.code, nom: f.nom, contact: f.contact || "", telephone: f.telephone || "", email: f.email || "", adresse: f.adresse || "", article_code: f.article_code || "", article_nom: f.article_nom || "", delai_livraison_jours: f.delai_livraison_jours, actif: f.actif });
        ...
        {tab === "receptions" && (
          <div style={s.section}>
            ...
            <div style={{ ...s.alertBox(C.tealLt, "#99f6e4", C.teal), marginBottom: "1.25rem" }}>
